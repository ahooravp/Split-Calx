...
const fetchTrips = useCallback(async (isFirstLoad = false) => {
  ...
  return (
    // Fixed parent wrapper transition logic to stop text color cascade bleed
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 relative flex flex-col overflow-hidden transition-[background-color,border-color,box-shadow] duration-300 ease-in-out pt-16">
      ...
      {/* Added transition-all duration-300 ease-in-out to synchronize blobs */}
      <div className="absolute top-[-10%] left-[-5%] w-[30rem] md:w-[40rem] h-[30rem] md:h-[40rem] bg-indigo-200 dark:bg-indigo-900/40 rounded-full mix-blend-multiply dark:mix-blend-screen filter blur-3xl opacity-40 pointer-events-none transition-all duration-300 ease-in-out"></div>
      <div className="absolute top-[40%] right-[-5%] w-[30rem] md:w-[40rem] h-[30rem] md:h-[40rem] bg-teal-200 dark:bg-teal-900/40 rounded-full mix-blend-multiply dark:mix-blend-screen filter blur-3xl opacity-40 pointer-events-none transition-all duration-300 ease-in-out"></div>
      <div className="absolute bottom-[-10%] left-[10%] w-[30rem] md:w-[40rem] h-[30rem] md:h-[40rem] bg-emerald-200 dark:bg-emerald-900/40 rounded-full mix-blend-multiply dark:mix-blend-screen filter blur-3xl opacity-40 pointer-events-none transition-all duration-300 ease-in-out"></div>
      ...
      <div className="pt-12 pb-8 px-6 sm:px-12 relative z-10 ">
        <div className="max-w-5xl mx-auto flex flex-col sm:flex-row sm:items-end justify-between gap-6 mt-2">
          <div>
            {/* Swapped to explicit transition properties to avoid lag */}
            <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-indigo-50/80 dark:bg-indigo-900/30 backdrop-blur-sm border border-indigo-200 dark:border-indigo-800 text-indigo-900 dark:text-indigo-300 font-semibold text-xs mb-4 shadow-xs transition-[background-color,border-color,box-shadow] duration-300 ease-in-out">
              <span className="flex h-1.5 w-1.5 rounded-full bg-indigo-600 dark:bg-indigo-400 animate-pulse"></span>
              Welcome back, {currentUser?.name}
            </div>
            <h2 className="text-4xl sm:text-5xl font-extrabold text-slate-900 dark:text-white tracking-tight transition-colors duration-300 ease-in-out">Dashboard</h2>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-5xl mx-auto px-6 sm:px-12 pb-16 relative z-10">
        <div className="grid grid-cols-1 md:grid-cols-12 gap-6 items-start">
          ...
          <div className="md:col-span-5 bg-white/90 dark:bg-slate-900/80 backdrop-blur-xl p-6 sm:p-8 rounded-3xl shadow-[0_20px_50px_rgb(0,0,0,0.04)] dark:shadow-[0_20px_50px_rgba(0,0,0,0.4)] border border-white/60 dark:border-slate-700/50 flex flex-col justify-between transition-[background-color,border-color,box-shadow] duration-300 ease-in-out">
            <div>
              <h3 className="font-bold text-2xl text-slate-900 dark:text-white mb-2 transition-colors duration-300 ease-in-out">Start a New Trip</h3>
              <p className="text-sm text-slate-500 dark:text-slate-400 mb-8 leading-relaxed transition-colors duration-300 ease-in-out">Create a dedicated hub to track expenses and split the bill with your friends.</p>
            </div>

            <form onSubmit={handleCreateTrip} className="flex flex-col gap-4">
              {/* Added missing duration and easing */}
              <input
                type="text"
                ...
                className="bg-slate-50/50 dark:bg-slate-950/50 border border-slate-200 dark:border-slate-800 p-4 rounded-xl w-full outline-none focus:bg-white dark:focus:bg-slate-900 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 font-medium text-slate-700 dark:text-slate-200 placeholder-slate-400 dark:placeholder-slate-500 transition-all duration-300 ease-in-out"
              />
              <button
                type="submit"
                disabled={isLoading}
          ...
          <div className="md:col-span-7 bg-white/90 dark:bg-slate-900/80 backdrop-blur-xl p-6 sm:p-8 rounded-3xl shadow-[0_20px_50px_rgb(0,0,0,0.04)] dark:shadow-[0_20px_50px_rgba(0,0,0,0.4)] border border-white/60 dark:border-slate-700/50 transition-[background-color,border-color,box-shadow] duration-300 ease-in-out">
            ...
            {isInitializing ? (
              ...
              <div className="flex flex-col items-center justify-center text-center py-12 bg-indigo-50/30 dark:bg-slate-800/30 rounded-2xl border border-dashed border-indigo-200 dark:border-slate-700 transition-[background-color,border-color] duration-300 ease-in-out">
                ...
                <p className="text-slate-600 dark:text-slate-400 font-bold mb-1 transition-colors duration-300 ease-in-out">No trips found</p>
                <p className="text-slate-400 dark:text-slate-500 text-sm font-medium transition-colors duration-300 ease-in-out">Create your first trip to get started.</p>
              </div>
            ) : (
              <ul className="space-y-3 max-h-[350px] overflow-y-auto pr-2 custom-scrollbar pb-2">
                {trips.map((trip) => (
                  <li
                    ...
                    /* Stripped duration-200 and mapped to 300 ease-in-out */
                    className="p-4 bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-700/50 rounded-2xl cursor-pointer hover:bg-white dark:hover:bg-slate-800 hover:border-indigo-200 dark:hover:border-indigo-800 hover:shadow-sm flex justify-between items-center transition-[background-color,border-color,box-shadow] duration-300 ease-in-out group"
                  >
                    <div className="flex items-center gap-4">
                      <span className="font-bold text-slate-700 dark:text-slate-200 text-lg group-hover:text-indigo-700 dark:group-hover:text-indigo-400 transition-colors duration-300 ease-in-out">{trip.name}</span>
                    </div>
                    ...
                    <div className="flex items-center">
                      ...
                      <span className="text-slate-300 dark:text-slate-600 group-hover:text-indigo-500 dark:group-hover:text-indigo-400 text-xl transform group-hover:translate-x-1 transition-all duration-300 ease-in-out">&rarr;</span>
                    </div>
                  </li>
                ))}
              </ul>
